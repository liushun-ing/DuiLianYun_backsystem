    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 获奖作品入库管理
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
export default {
    ...
    methods: {
        ...
        saveEdit (index, row) {
            ...
            var that = this;
            this.$confirm('确定要进入该活动获奖作品入库管理吗？', '提示', {
                type: 'warning'
            })
                .then(() => {
                    this.$axios({
                        method: 'get',
                        url: this.GLOBAL.baseURL + '/couplet-server/couplet/setCurrentEventId2?eventId=' + this.eventList[index].eventId,
                        headers: {
                            'token': this.GLOBAL.token.token
                        }
                    })
                        .then(function (response) {
                            ...
                            that.getPage(false);
                            that.$router.replace('/OneActivityAwardManage');
                            Utils.$emit('demo','msg');
                            that.$message({
                                message: '进入成功',
                                type: 'success'
