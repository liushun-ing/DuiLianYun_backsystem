    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="displayManuscriptList" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                <el-table-column label='投稿内容' align='center' min-width="300">
                ...
                <el-table-column prop="manuscriptCreateTime" label="投稿时间" width="160" sortable></el-table-column>
                
                <el-table-column label="二审分数" align="center" min-width="100">
                    <template slot-scope="scope" >
                        <div v-if='scope.row.secondScore != -1'>{{scope.row.secondScore}}</div>
                    </template>
                </el-table-column>
                <el-table-column label="二审评语" prop="secondComment" align="center" min-width="200"></el-table-column>
                <el-table-column label="操作" width="100" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button v-if='scope.row.secondScore == -1' type="text" icon="el-icon-edit" class="blue" @click="handleReview(scope.$index, scope.row)"
                            >作品评审</el-button
                        >
...
export default {
    ...
    methods: {
        ...
        async handleSelected() {
            this.currentPage = 1;
            this.getSecondReviewManuscript();
            // let _this = this;
                    ...
                    try {  
                        ...
                        for(let i = 0; i < this.displayManuscriptList.length; i ++){
                            if(this.displayManuscriptList[i].manuscriptId === this.reviewData.manuscriptId) {
                                ...
                                this.$set(this.displayManuscriptList[i], 'secondComment', this.reviewData.comment);
                            }
                        }
                        for(let i = 0; i < this.manuscriptList.length; i ++){
                            if(this.manuscriptList[i].manuscriptId === this.reviewData.manuscriptId) {
                                ...
                                this.$set(this.manuscriptList[i], 'secondComment', this.reviewData.comment);
                            }
                        }
                        this.reviewData = {
                            reviewerId: '',
